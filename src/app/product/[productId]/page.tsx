...
const page = async ({ params }: PageProps) => {
    ...
    return (
        ...
        <MaxWidthWrapper classname='bg-white'>
                ...
                <div className='mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:gap-x-8 lg:px-8'>
                    ...
                    <div className='lg:max-w-lg lg:self-end'>
                        <ol className='flex items-center space-x-2'>
                            {BREADCRUMBS.map((breadcrupmb, i) => (
                                <li key={breadcrupmb.href}>
                                    <div className='flex items-center text-sm'>
                                        <Link href={breadcrupmb.href} className='font-medium text-sm text-muted-foreground hover:text-gray-700'>
                                            {breadcrupmb.name}
            ...
            <ProductReel href='/products' query={{ catagory: product.catagory, limit: 4 }} title={`Similar ${label}`} subtitle={`Brows similar high-quality ${label} just like ${product.name}`} />
        </MaxWidthWrapper>
